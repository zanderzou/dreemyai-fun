import { localeList, type LocaleSlug } from "./localized-dreemy";
...
const articlePaths = new Set(comparisonKeys.map((key) => `/blog/dreemy-ai-vs-${key}/`));
...
export function languageAlternates(pathname:string):{code:string;href:string}[] {
  ...
  return [
    ...locales.map((locale)=>({code:locale==="en"?"en":localeList.find((entry)=>entry.slug===locale)!.code,href:`${site.url}${localePath(locale,pathname)}`})),
